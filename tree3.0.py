        ...
        while change:
            ...
            for name, player in leaves_copy.items() :
                ...
                if player != self.current_player and names[names[name].father].num_subbranch == 1:
                    ...
                    try:
                        names[names[name_pa].father].name_subbranch.remove(name_pa)
                    ...
                    del leaves[name]
                    # temp = names[names[name_pa].father]
                    # while temp.num_subbranch == 0 and temp!=self:
                    #     try:
                    #         names[temp.father].name_subbranch.remove(temp.name)
                    #     except:
                    #         a=1
                    #     names[temp.father].num_subbranch -= 1
                    #     black_branch.append(temp.name)
                    #     temp = names[temp.father]
...
while 1:
    ...
    if j == 0:
        ...
        for name in current_branch.name_subbranch:
            for i in range(500//current_branch.num_subbranch):
                names[name].expend1()
